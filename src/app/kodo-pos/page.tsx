...
export default function KodoPosPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 relative overflow-hidden font-['Plus_Jakarta_Sans']">

      {/* Background glow effects */}
      <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-[var(--accent-glow)] blur-[150px] rounded-full pointer-events-none" />
      ...
      >
        <div className="text-center mb-10">
          ...
          <h1 className="font-['Outfit'] text-4xl md:text-5xl font-black tracking-tight mb-4 bg-clip-text text-transparent bg-gradient-to-b from-foreground to-foreground/70">
            Obtenir Kōdo POS
          </h1>
        ...
        <AnimatePresence mode="wait">
          {!success ? (
            ...
            >
              ...
              {/* OS Selector */}
              <div className="flex p-1 bg-surface-variant/50 rounded-2xl mb-8">
                <button
                  type="button"
                  onClick={() => setOs('macOS')}
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-all ${os === 'Windows' ? 'bg-foreground text-background shadow-lg' : 'text-foreground/50 hover:text-foreground hover:bg-foreground/5'}`}
                >
                  <Monitor size={18} className={os === 'Windows' ? 'fill-background text-background' : ''} /> Windows
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-4">
                  <div className="relative group">
                    <input type="text" name="shop_name" required value={formData.shop_name} onChange={handleChange} placeholder="Nom de l'établissement" className="w-full bg-surface-variant/50 border border-outline rounded-xl px-5 py-4 text-foreground placeholder-foreground/30 focus:outline-none focus:border-[var(--accent)] focus:bg-surface transition-all" />
                  </div>
                  <div className="relative group">
                    <input type="text" name="full_name" required value={formData.full_name} onChange={handleChange} placeholder="Prénom & Nom" className="w-full bg-surface-variant/50 border border-outline rounded-xl px-5 py-4 text-foreground placeholder-foreground/30 focus:outline-none focus:border-[var(--accent)] focus:bg-surface transition-all" />
                  </div>
                  <div className="relative group">
                    <input type="email" name="email" required value={formData.email} onChange={handleChange} placeholder="Email professionnel" className="w-full bg-surface-variant/50 border border-outline rounded-xl px-5 py-4 text-foreground placeholder-foreground/30 focus:outline-none focus:border-[var(--accent)] focus:bg-surface transition-all" />
                  </div>
                  <div className="relative group">
                    <input type="tel" name="phone" required value={formData.phone} onChange={handleChange} placeholder="Numéro de téléphone" className="w-full bg-surface-variant/50 border border-outline rounded-xl px-5 py-4 text-foreground placeholder-foreground/30 focus:outline-none focus:border-[var(--accent)] focus:bg-surface transition-all" />
                  </div>
                </div>
                ...
                <div className="pt-4">
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full relative group overflow-hidden bg-foreground text-background font-bold text-lg rounded-xl py-4 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:hover:scale-100 flex items-center justify-center gap-2 shadow-[0_0_20px_var(--accent-glow)]"
                  >
                    {/* Button hover glow */}
                ...
                <p className="text-center text-xs text-foreground/40 mt-4 flex items-center justify-center gap-3">
                  ...
                  <span>|</span>
                  <span className="flex items-center gap-1"><CheckCircle2 size={12} className="text-[var(--accent)]" /> Mode Hors-Ligne Assuré</span>
                </p>
              </form>
            </motion.div>
          ) : (
            ...
            >
              ...
              <h2 className="font-['Outfit'] text-3xl font-bold mb-4 text-foreground">Votre téléchargement est prêt.</h2>
              ...
              <div className="bg-surface-variant/50 border border-outline rounded-2xl p-6 text-left">
